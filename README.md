...
1. ## <a name="_toc1908440452"></a>**Tema do Projeto**
...
1. ## <a name="_toc672272216"></a>**Usuários do sistema**  
...
1. ## ` `**<a name="_toc1351567607"></a>Funcionalidade 1 do Projeto**
...
1. ## ` `**<a name="_toc1281303573"></a>Funcionalidade 2 do Projeto**
Indicar uma funcionalidade (requisito funcional) que deve ser implementada para atender uma necessidade do usuário, utilizando os dados em BD.
...
1. # <a name="_toc42559530"></a>**Modelo Conceitual**
Imagem de boa resolução do Modelo Conceitual (**Modelo Entidade-Relacionamento - MER**) na **3FN**

![Diagrama

Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.002.png)
Projeto BD		CineView							4
||||
| :- | :-: | -: |

1. # <a name="_toc2133822257"></a>**Modelo Lógico (3FN)**
Imagem de boa resolução do Modelo Lógico (**Modelo Relacional - MR**) na **3FN**, feito com o **brModelo**.

![Diagrama

Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.003.png)
1. # <a name="_toc1392738563"></a>**Modelo Lógico (3FN)**
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.004.png)
Projeto BD		CineView							6
||||
| :- | :-: | -: |

1. # <a name="_toc663449489"></a>**Modelo Físico (3FN)**
   1. ## <a name="_toc685451885"></a>**SQL para criação de tabelas e restrições** 
CREATE TABLE Usuario ( 

`    `ID\_Usuario INT AUTO\_INCREMENT PRIMARY KEY, 

`    `Nome VARCHAR(50) NOT NULL, 

`    `Email VARCHAR(50) DEFAULT ('Não informado') UNIQUE, 

`    `Dt\_Nasc DATE DEFAULT ('Não informada'), 

`    `Sexo VARCHAR(10) DEFAULT ('Prefiro não dizer'), 

`    `fk\_GeneroFav INT NOT NULL); 

CREATE TABLE Filme ( 

`    `ID\_Filme INT AUTO\_INCREMENT PRIMARY KEY, 
...
`    `fk\_ID\_Genero INT NOT NULL, 

`    `fk\_ID\_Diretor INT DEFAULT ('Desconhecido'), 

`    `fk\_ID\_Estudio INT DEFAULT ('Desconhecido')); 

CREATE TABLE Genero ( 

`    `ID\_Genero INT AUTO\_INCREMENT PRIMARY KEY, 

`    `Nome VARCHAR(50) NOT NULL); 

CREATE TABLE Diretor ( 

`    `ID\_Diretor INT AUTO\_INCREMENT PRIMARY KEY, 

`    `Nome VARCHAR(50) NOT NULL, 

`    `Dt\_Nasc DATE DEFAULT ('Desconhecida'), 

`    `Nacionalidade VARCHAR(50) DEFAULT ('Desconhecida')); 

CREATE TABLE Estudio ( 

`    `ID\_Estudio INT AUTO\_INCREMENT PRIMARY KEY, 
...
`    `ID\_Ator INT AUTO\_INCREMENT PRIMARY KEY, 
...
`    `ID\_Avaliacao INT AUTO\_INCREMENT PRIMARY KEY, 
...
`    `fk\_ID\_Usuario INT NOT NULL, 

`    `fk\_ID\_Filme INT NOT NULL); 

CREATE TABLE Elenco ( 

`    `ID\_Atuacao INT AUTO\_INCREMENT PRIMARY KEY,

`    `fk\_ID\_Filme INT NOT NULL,

`    `fk\_ID\_Ator INT NOT NULL),

`    `UNIQUE (fk\_ID\_Filme, fk\_ID\_Ator));

ALTER TABLE Usuario ADD CONSTRAINT FK\_Usuario\_2 

`    `FOREIGN KEY (fk\_GeneroFav) 

`    `REFERENCES Genero (ID\_Genero) 

`    `ON DELETE RESTRICT;  

ALTER TABLE Filme ADD CONSTRAINT FK\_Filme\_2 

`    `FOREIGN KEY (fk\_ID\_Genero) 

`    `REFERENCES Genero (ID\_Genero) 

`    `ON DELETE RESTRICT;

ALTER TABLE Filme ADD CONSTRAINT FK\_Filme\_3 

`    `FOREIGN KEY (fk\_ID\_Diretor) 

`    `REFERENCES Diretor (ID\_Diretor) 
...
ALTER TABLE Filme ADD CONSTRAINT FK\_Filme\_4 

`    `FOREIGN KEY (fk\_ID\_Estudio) 

`    `REFERENCES Estudio (ID\_Estudio) 

`    `ON DELETE RESTRICT; 

ALTER TABLE Avalia ADD CONSTRAINT FK\_Avalia\_2 

`    `FOREIGN KEY (fk\_ID\_Usuario) 

`    `REFERENCES Usuario (ID\_Usuario) 

`    `ON DELETE RESTRICT;  

ALTER TABLE Avalia ADD CONSTRAINT FK\_Avalia\_3 

`    `FOREIGN KEY (fk\_ID\_Filme) 

`    `REFERENCES Filme (ID\_Filme) 

`    `ON DELETE RESTRICT; 

ALTER TABLE Elenco ADD CONSTRAINT FK\_Elenco\_2 
...
ALTER TABLE Elenco ADD CONSTRAINT FK\_Elenco\_3 

`    `FOREIGN KEY (fk\_ID\_Ator) 

`    `REFERENCES Ator (ID\_Ator) 
...
1. ## <a name="_toc1777438470"></a>**SQL para inserção de pelo menos 15 registros para cada tabela**  
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.008.png)
...
Descrição gerada automaticamente com confiança média](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.009.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.010.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.011.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.012.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.013.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.014.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.015.png)
1. # <a name="_toc1388994099"></a>**Consultas e Programação**
...
1) Resultado da consulta.

![Tabela

Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.016.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.017.png)
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.018.png)

1. ## <a name="_toc559110447"></a>**SQL para 3 consultas com IR (PK + FK) de recuperação de dados** 
**Consulta 1**: 
...
INNER JOIN Genero AS G ON F.fk\_ID\_Genero = G.ID\_Genero

INNER JOIN Diretor AS D ON F.fk\_ID\_Diretor = D.ID\_Diretor

INNER JOIN Estudio AS E ON F.fk\_ID\_Estudio = E.ID\_Estudio;
...
INNER JOIN Usuario AS U ON A.fk\_ID\_Usuario = U.ID\_Usuario

INNER JOIN Filme AS F ON A.fk\_ID\_Filme = F.ID\_Filme

WHERE A.fk\_ID\_Filme = 4; -- ID do filme desejado
...
`    `DATE\_FORMAT(A.Dt\_Nasc, '%d/%m/%Y') AS 'Data de Nascimento',

`    `A.Nacionalidade AS 'Nacionalidade',

`    `TIMESTAMPDIFF(YEAR, A.Dt\_Nasc, NOW()) AS Idade

FROM Ator AS A

INNER JOIN Elenco AS E ON A.ID\_Ator = E.fk\_ID\_Ator

INNER JOIN Filme AS F ON E.fk\_ID\_Filme = F.ID\_Filme

WHERE E.fk\_ID\_Filme = 1; -- ID do filme desejado
...
Descrição gerada automaticamente](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.024.png)
...
1. ## <a name="_toc1123440608"></a>**1 Stored procedure**
**SP 1**: 
...
1) Descreva por qual razão a SP é importante para o usuário.
...
CREATE PROCEDURE CalculaMediaAvaliacao(IN ID\_Filme INT, OUT MediaAvaliacao FLOAT, OUT NomeFilme VARCHAR(50))
...
`    `WHERE A.fk\_ID\_Filme = ID\_Filme;
...
CALL CalculaMediaAvaliacao(6, @MediaAvaliacao, @NomeFilme);
...
1. ## <a name="_toc780368612"></a>**1 Trigger**
**Trigger 1**: 
...
1) Descreva por qual razão o *Trigger* é importante para o usuário.
...
`        `SET MESSAGE\_TEXT = 'Ocorreu um erro! Não é possível fazer uma avaliação com esse valor.';
...
INSERT INTO Avalia (Nota, fk\_ID\_Usuario, fk\_ID\_Filme) VALUES (13, 1, 3); -- Exemplo inválido

INSERT INTO Avalia (Nota, fk\_ID\_Usuario, fk\_ID\_Filme) VALUES (9, 1, 3); -- Exemplo válido

![](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.027.png)

1) Resultado da execução do *Trigger*

![](files/Aspose.Words.ba37472d-a545-40c5-936a-ec2541f212cd.028.png)
Projeto BD	CineView	24
